---
title: "Tasks | Tasks - Run Commands in Zed"
source_url: "https://zed.dev/docs/tasks"
...
Zed supports ways to spawn (and rerun) commands using its integrated [terminal](https://zed.dev/docs/terminal.html) to output the results. These commands can read a limited subset of Zed state (such as a path to the file currently being edited or selected text).
...
## [Task templates](https://zed.dev/docs/tasks.html#task-templates)
...
* on the fly with [oneshot tasks](https://zed.dev/docs/tasks.html#oneshot-tasks). These tasks are project-specific and do not persist across sessions.
* by language extension.

## [Variables](https://zed.dev/docs/tasks.html#variables)
...
* `ZED_CUSTOM_RUST_PACKAGE`: (Rust-specific) name of the parent package of $ZED\_FILE source file.
...
### [Variable Quoting](https://zed.dev/docs/tasks.html#variable-quoting)
...
### [Task filtering based on variables](https://zed.dev/docs/tasks.html#task-filtering-based-on-variables)
...
## [Oneshot tasks](https://zed.dev/docs/tasks.html#oneshot-tasks)
...
### [Ephemeral tasks](https://zed.dev/docs/tasks.html#ephemeral-tasks)
...
### [More task rerun control](https://zed.dev/docs/tasks.html#more-task-rerun-control)
...
## [Custom keybindings for tasks](https://zed.dev/docs/tasks.html#custom-keybindings-for-tasks)

You can define your own keybindings for your tasks via an additional argument to `task::Spawn`. If you wanted to bind the aforementioned `echo current file's path` task to `alt-g`, you would add the following snippet in your [`keymap.json`](https://zed.dev/docs/key-bindings.html) file:
...
## [VS Code Task Format](https://zed.dev/docs/tasks.html#vs-code-task-format)
...
## [Binding runnable tags to task templates](https://zed.dev/docs/tasks.html#binding-runnable-tags-to-task-templates)
...
## [Keybindings to run tasks bound to runnables](https://zed.dev/docs/tasks.html#keybindings-to-run-tasks-bound-to-runnables)

When you have a task definition that is bound to the runnable, you can quickly run it using [Code Actions](https://zed.dev/docs/configuring-languages%3F.html#code-actions) that you can trigger either via `editor: Toggle Code Actions` command or by the `cmd-.`/`ctrl-.` shortcut. Your task will be the first in the dropdown. The task will run immediately if there are no additional Code Actions for this line.

## [Running Bash Scripts](https://zed.dev/docs/tasks.html#running-bash-scripts)
...
## [Shell Initialization](https://zed.dev/docs/tasks.html#shell-initialization)
...
See [Terminal configuration](https://zed.dev/docs/terminal.html) for complete shell options.
