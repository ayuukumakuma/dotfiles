---
title: "default_domain - Wez's Terminal Emulator"
source_url: "https://wezterm.org/config/lua/config/default_domain"
...
# `default_domain = "local"`[¶](https://wezterm.org/config/lua/config/default_domain.html#default_domain-local "Permanent link")
...
the standalone mux server, see [default\_mux\_server\_domain](https://wezterm.org/config/lua/config/default_mux_server_domain.html)
...
`wsl.exe`. Using a [WslDomain](https://wezterm.org/config/lua/WslDomain.html) for this has the advantage
that wezterm can then use [shell integration](https://wezterm.org/shell-integration.html) to
...
of any of the available [multiplexing domains](https://wezterm.org/multiplexing.html).
