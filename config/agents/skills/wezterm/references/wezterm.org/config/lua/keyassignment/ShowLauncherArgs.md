---
title: "ShowLauncherArgs - Wez's Terminal Emulator"
source_url: "https://wezterm.org/config/lua/keyassignment/ShowLauncherArgs"
...
# `ShowLauncherArgs`[¶](https://wezterm.org/config/lua/keyassignment/ShowLauncherArgs.html#showlauncherargs "Permanent link")
...
Activate the [Launcher Menu](https://wezterm.org/config/launch.html#the-launcher-menu)
...
* `help_text` - a string to display when in the default mode. Defaults to:
  `"Select an item and press Enter=launch Esc=cancel /=filter"` (*Since: Nightly Builds Only*)
* `fuzzy_help_text` - a string to display when in fuzzy finding mode. Defaults to:
  `"Fuzzy matching: "` (*Since: Nightly Builds Only*)
* `alphabet` - a string of unique characters. The characters in the string are used
  ...
  [launcher\_alphabet](https://wezterm.org/config/lua/config/launcher_alphabet.html) (*Since: Nightly Builds Only*)
...
* `"LAUNCH_MENU_ITEMS"` - include the [launch\_menu](https://wezterm.org/config/lua/config/launch_menu.html) items
...
* `"COMMANDS"` - include a number of default commands (*Since: Version 20220408-101518-b908e2dd*)
