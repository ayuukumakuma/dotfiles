---
title: "QuickSelectArgs - Wez's Terminal Emulator"
source_url: "https://wezterm.org/config/lua/keyassignment/QuickSelectArgs"
...
# `QuickSelectArgs`[¶](https://wezterm.org/config/lua/keyassignment/QuickSelectArgs.html#quickselectargs "Permanent link")
...
Activates [Quick Select Mode](https://wezterm.org/quickselect.html) but with the option
...
the default or the [quick\_select\_patterns](https://wezterm.org/config/lua/config/quick_select_patterns.html)
configuration:
...
* `alphabet` - if present, this alphabet is used instead of [quick\_select\_alphabet](https://wezterm.org/config/lua/config/quick_select_alphabet.html)
* `action` - if present, this key assignment action is performed as if by [window:perform\_action](https://wezterm.org/config/lua/window/perform_action.html) when an item is selected. The normal clipboard action is NOT performed in this case.
* `skip_action_on_paste` - overrides whether `action` is performed after an item is selected using a capital value (when paste occurs). (*Since: Nightly Builds Only*)
...
* `scope_lines` - Specify the number of lines to search above and below the current viewport. The default is 1000 lines. The scope will be increased to the current viewport height if it is smaller than the viewport. (*Since: Version 20220807-113146-c2fee766*). In earlier releases, the entire scrollback was always searched).
...
See also [wezterm.open\_with](https://wezterm.org/config/lua/wezterm/open_with.html).
