---
title: "Run | The Unified Toolchain for the Web"
source_url: "https://viteplus.dev/guide/run"
fetched_at: "2026-03-31T02:50:28.336963+00:00"
---



# Run [​](https://viteplus.dev/guide/run.html#run)
...
## Overview [​](https://viteplus.dev/guide/run.html#overview)
...
## Caching [​](https://viteplus.dev/guide/run.html#caching)
...
## Task Definitions [​](https://viteplus.dev/guide/run.html#task-definitions)
...
Tasks defined in `vite.config.ts` are cached by default. `package.json` scripts are not. See [When Is Caching Enabled?](https://viteplus.dev/guide/cache.html#when-is-caching-enabled) for the full resolution order.

See [Run Config](https://viteplus.dev/config/run.html) for the full `run` block reference.

## Task Dependencies [​](https://viteplus.dev/guide/run.html#task-dependencies)

Use [`dependsOn`](https://viteplus.dev/guide/run.html#depends-on) to run tasks in the right order. Running `vp run deploy` with the config above runs `build` and `test` first. Dependencies can also target other packages in the same project with the `package#task` notation:
...
## Running in a Workspace [​](https://viteplus.dev/guide/run.html#running-in-a-workspace)
...
### Recursive (`-r`) [​](https://viteplus.dev/guide/run.html#recursive-r)
...
### Transitive (`-t`) [​](https://viteplus.dev/guide/run.html#transitive-t)
...
### Filter (`--filter`) [​](https://viteplus.dev/guide/run.html#filter-filter)
...
### Workspace Root (`-w`) [​](https://viteplus.dev/guide/run.html#workspace-root-w)
...
## Compound Commands [​](https://viteplus.dev/guide/run.html#compound-commands)

Commands joined with `&&` are split into independent sub-tasks. Each sub-task is cached separately when [caching is enabled](https://viteplus.dev/guide/cache.html#when-is-caching-enabled). This works for both `vite.config.ts` tasks and `package.json` scripts:
...
### Nested `vp run` [​](https://viteplus.dev/guide/run.html#nested-vp-run)
...
## Execution Summary [​](https://viteplus.dev/guide/run.html#execution-summary)
...
## Additional Arguments [​](https://viteplus.dev/guide/run.html#additional-arguments)
