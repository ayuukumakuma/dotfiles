---
title: "Vim Mode | Vim Mode - Zed"
source_url: "https://zed.dev/docs/vim"
...
## [Zed's vim mode design](https://zed.dev/docs/vim.html#zeds-vim-mode-design)
...
### [Core differences](https://zed.dev/docs/vim.html#core-differences)
...
4. **Search and replace**: vim mode uses Zed's search system, so, the syntax for regular expressions is slightly different compared to Vim. [Head to the Regex differences section](https://zed.dev/docs/vim.html#regex-differences) for details.
...
## [Enabling and disabling vim mode](https://zed.dev/docs/vim.html#enabling-and-disabling-vim-mode)
...
## [Zed-specific features](https://zed.dev/docs/vim.html#zed-specific-features)
...
### [Language server](https://zed.dev/docs/vim.html#language-server)
...
### [Git](https://zed.dev/docs/vim.html#git)
...
### [Tree-sitter](https://zed.dev/docs/vim.html#tree-sitter)
...
### [Multi cursor](https://zed.dev/docs/vim.html#multi-cursor)
...
### [Pane management](https://zed.dev/docs/vim.html#pane-management)
...
### [In insert mode](https://zed.dev/docs/vim.html#in-insert-mode)
...
### [Supported plugins](https://zed.dev/docs/vim.html#supported-plugins)
...
* You can add key bindings to your keymap to navigate "camelCase" names. [Head down to the Optional key bindings](https://zed.dev/docs/vim.html#optional-key-bindings) section to learn how.
* You can use `gR` to do [ReplaceWithRegister](https://github.com/vim-scripts/ReplaceWithRegister).
* You can use `cx` for [vim-exchange](https://github.com/tommcdo/vim-exchange) functionality. Note that it does not have a default binding in visual mode, but you can add one to your keymap (refer to the [optional key bindings](https://zed.dev/docs/vim.html#optional-key-bindings) section).
...
* You can select quoted text with AnyQuotes and bracketed text with AnyBrackets text objects. Zed also provides MiniQuotes and MiniBrackets which offer alternative selection behavior based on the [mini.ai](https://github.com/echasnovski/mini.nvim/blob/main/readmes/mini-ai.md) Neovim plugin. See the [Quote and Bracket text objects](https://zed.dev/docs/vim.html#quote-and-bracket-text-objects) section below for details.
* You can configure AnyQuotes, AnyBrackets, MiniQuotes, and MiniBrackets text objects for selecting quoted and bracketed text using different selection strategies. See the [Any Bracket Functionality](https://zed.dev/docs/vim.html#any-bracket-functionality) section below for details.

### [Any Bracket Functionality](https://zed.dev/docs/vim.html#any-bracket-functionality)
...
#### [Included Characters](https://zed.dev/docs/vim.html#included-characters)
...
#### [AnyQuotes and AnyBrackets (Traditional Vim behavior)](https://zed.dev/docs/vim.html#anyquotes-and-anybrackets-traditional-vim-behavior)
...
#### [MiniQuotes and MiniBrackets (mini.ai behavior)](https://zed.dev/docs/vim.html#miniquotes-and-minibrackets-miniai-behavior)
...
#### [Choosing Between Approaches](https://zed.dev/docs/vim.html#choosing-between-approaches)
...
#### [Example Configuration](https://zed.dev/docs/vim.html#example-configuration)
...
## [Command palette](https://zed.dev/docs/vim.html#command-palette)
...
### [File and window management](https://zed.dev/docs/vim.html#file-and-window-management)
...
### [Ex commands](https://zed.dev/docs/vim.html#ex-commands)
...
### [Navigating diagnostics](https://zed.dev/docs/vim.html#navigating-diagnostics)
...
### [Git](https://zed.dev/docs/vim.html#git-1)
...
### [Jump](https://zed.dev/docs/vim.html#jump)
...
### [Replacement](https://zed.dev/docs/vim.html#replacement)

This command replaces text. It emulates the substitute command in vim. The substitute command uses regular expressions, and Zed uses a slightly different syntax than vim. You can learn more about Zed's syntax below, [in the regex differences section](https://zed.dev/docs/vim.html#regex-differences). Zed will replace only the first occurrence of the search pattern in the current line. To replace all matches append the `g` flag.

| Command | Description |
| --- | --- |
| `:[range]s/foo/bar/[g]` | Replace instances of foo with bar |

### [Editing](https://zed.dev/docs/vim.html#editing)
...
### [Set](https://zed.dev/docs/vim.html#set)
...
### [Command mnemonics](https://zed.dev/docs/vim.html#command-mnemonics)
...
## [Customizing key bindings](https://zed.dev/docs/vim.html#customizing-key-bindings)

### [Selecting the correct context](https://zed.dev/docs/vim.html#selecting-the-correct-context)
...
| vim\_mode == normal | Normal mode |
| vim\_mode == visual | Visual mode |
| vim\_mode == insert | Insert mode |
| vim\_mode == replace | Replace mode |
| vim\_mode == waiting | Waiting for an arbitrary key (e.g., after typing `f` or `t`) |
| vim\_mode == operator | Waiting for another binding to trigger (e.g., after typing `c` or `d`) |
| vim\_operator | Set to `none` unless `vim_mode == operator`, in which case it is set to the current operator's default keybinding (e.g., after typing `d`, `vim_operator == d`) |
...
### [Useful contexts for vim mode key bindings](https://zed.dev/docs/vim.html#useful-contexts-for-vim-mode-key-bindings)
...
### [Optional key bindings](https://zed.dev/docs/vim.html#optional-key-bindings)
...
### [Restoring common text editing and Zed keybindings](https://zed.dev/docs/vim.html#restoring-common-text-editing-and-zed-keybindings)
...
## [Changing vim mode settings](https://zed.dev/docs/vim.html#changing-vim-mode-settings)

You can change the following settings to modify vim mode's behavior:

| Property | Description | Default Value |
| --- | --- | --- |
| default\_mode | The default mode to start in. One of "normal", "insert", "replace", "visual", "visual\_line", "visual\_block", "helix\_normal". | "normal" |
| use\_system\_clipboard | Determines how system clipboard is used:   * "always": use for all operations * "never": only use when explicitly specified * "on\_yank": use for yank operations | "always" |
| use\_multiline\_find | deprecated |  |
| use\_smartcase\_find | If `true`, `f` and `t` motions are case-insensitive when the target letter is lowercase. | false |
...
| toggle\_relative\_line\_numbers | If `true`, line numbers are relative in normal mode and absolute in insert mode, giving you the best of both options. | false |
| custom\_digraphs | An object that allows you to add custom digraphs. Read below for an example. | {} |
| highlight\_on\_yank\_duration | The duration of the highlight animation(in ms). Set to `0` to disable | 200 |
...
## [Useful core Zed settings for vim mode](https://zed.dev/docs/vim.html#useful-core-zed-settings-for-vim-mode)

Here are a few general Zed settings that can help you fine-tune your Vim experience:

| Property | Description | Default Value |
| --- | --- | --- |
| cursor\_blink | If `true`, the cursor blinks. | `true` |
| relative\_line\_numbers | If `"enabled"`, line numbers in the left gutter are relative to the cursor. If `"wrapped"`, they also display for wrapped lines. | `"disabled"` |
| scrollbar | Object that controls the scrollbar display. Set to `{ "show": "never" }` to hide the scroll bar. | `{ "show": "auto" }` |
| scroll\_beyond\_last\_line | If set to `"one_page"`, allows scrolling up to one page beyond the last line. Set to `"off"` to prevent this behavior. | `"one_page"` |
| vertical\_scroll\_margin | The number of lines to keep above or below the cursor when scrolling. Set to `0` to allow the cursor to go up to the edges of the screen vertically. | `3` |
| gutter.line\_numbers | Controls the display of line numbers in the gutter. Set the `"line_numbers"` property to `false` to hide line numbers. | `true` |
| command\_aliases | Object that defines aliases for commands in the command palette. You can use it to define shortcut names for commands you use often. Read below for examples. | `{}` |
...
## [Regex differences](https://zed.dev/docs/vim.html#regex-differences)
...
[Keybindings](https://zed.dev/docs/key-bindings.html "Keybindings")
