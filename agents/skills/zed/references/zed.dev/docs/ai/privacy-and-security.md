---
title: "Privacy and Security | AI Privacy and Security - Zed"
source_url: "https://zed.dev/docs/ai/privacy-and-security"
fetched_at: "2026-03-05T02:24:06.365855+00:00"
---



# [Privacy and Security](https://zed.dev/docs/ai/privacy-and-security.html#privacy-and-security)

## [Philosophy](https://zed.dev/docs/ai/privacy-and-security.html#philosophy)

Zed collects minimal data necessary to serve and improve our product. Features that could share data, like AI and telemetry, are either opt-in or can be disabled.

* **Telemetry**: Zed collects only the data necessary to understand usage and fix issues. Client-side telemetry can be disabled in settings.
* **AI**: Data sharing for AI improvement is opt-in, and each share is a one-time action; it does not grant permission for future data collection. You can use Zed's AI features without sharing any data with Zed and without authenticating.
* **Open-Source**: Zed's codebase is public. You can inspect exactly what data is collected and how it's handled. If you find issues, we encourage you to report them.
* **Secure-by-default**: Designing Zed and our Service with "secure-by-default" as an objective is of utmost importance to us. We take your security and ours very seriously and strive to follow industry best-practice in order to uphold that principle.

## [Related Documentation](https://zed.dev/docs/ai/privacy-and-security.html#related-documentation)

* [Tool Permissions](https://zed.dev/docs/ai/tool-permissions.html): Configure granular rules to control which agent actions are auto-approved, blocked, or require confirmation.
* [Worktree trust](https://zed.dev/docs/worktree-trust.html): How Zed opens files and directories in restricted mode.
* [Telemetry](https://zed.dev/docs/telemetry.html): How Zed collects general telemetry data.
* [Zed AI Features and Privacy](https://zed.dev/docs/ai/ai-improvement.html): An overview of Zed's AI features, your data when using AI in Zed, and how to opt-in and help Zed improve these features.
* [Accounts](https://zed.dev/docs/authentication.html): When and why you'd need to authenticate into Zed, how to do so, and what scope we need from you.
* [Collab](https://zed.dev/faq#data-and-privacy): How Zed's live collaboration works and how data flows. Zed does not store your code.

## [Legal Links](https://zed.dev/docs/ai/privacy-and-security.html#legal-links)

* [Terms of Service](https://zed.dev/terms)
* [Privacy Policy](https://zed.dev/privacy-policy)
* [Zed's Contributor License and Feedback Agreement](https://zed.dev/cla)
* [Subprocessors](https://zed.dev/subprocessors)

[Authenticate](https://zed.dev/docs/authentication.html "Authenticate")
[Worktree Trust](https://zed.dev/docs/worktree-trust.html "Worktree Trust")
