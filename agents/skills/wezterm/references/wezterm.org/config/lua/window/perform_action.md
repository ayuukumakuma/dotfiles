---
title: "perform_action - Wez's Terminal Emulator"
source_url: "https://wezterm.org/config/lua/window/perform_action"
...
# `window:perform_action(key_assignment, pane)`[¶](https://wezterm.org/config/lua/window/perform_action.html#windowperform_actionkey_assignment-pane "Permanent link")
...
The first parameter is a key assignment such as that returned by [`wezterm.action`](https://wezterm.org/config/lua/wezterm/action.html).

The second parameter is a `pane` object passed to your event callback.

For an example of this method in action, see [`wezterm.on` Custom Events](https://wezterm.org/config/lua/wezterm/on.html#custom-events).
