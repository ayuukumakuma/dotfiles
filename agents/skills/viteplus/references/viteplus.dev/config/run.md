---
title: "Run Config | The Unified Toolchain for the Web"
source_url: "https://viteplus.dev/config/run"
fetched_at: "2026-03-31T02:50:28.336963+00:00"
---



# Run Config [​](https://viteplus.dev/config/run.html#run-config)

You can configure Vite Task under the `run` field in `vite.config.ts`. Check out [`vp run`](https://viteplus.dev/guide/run.html) to learn more about running scripts and tasks with Vite+.
...
## `run.enablePrePostScripts` [​](https://viteplus.dev/config/run.html#run-enableprepostscripts)
...
## `run.cache` [​](https://viteplus.dev/config/run.html#run-cache)
...
## `run.tasks` [​](https://viteplus.dev/config/run.html#run-tasks)

* **Type:** `Record<string, TaskConfig>`

Defines tasks that can be run with `vp run <task>`.

### `command` [​](https://viteplus.dev/config/run.html#command)
...
Commands joined with `&&` are automatically split into independently cached sub-tasks. See [Compound Commands](https://viteplus.dev/guide/run.html#compound-commands).

### `dependsOn` [​](https://viteplus.dev/config/run.html#dependson)
...
See [Task Dependencies](https://viteplus.dev/guide/run.html#task-dependencies) for details on how explicit and topological dependencies interact.

### `cache` [​](https://viteplus.dev/config/run.html#cache)
...
### `env` [​](https://viteplus.dev/config/run.html#env)
...
### `untrackedEnv` [​](https://viteplus.dev/config/run.html#untrackedenv)
...
### `input` [​](https://viteplus.dev/config/run.html#input)

* **Type:** `Array<string | { auto: boolean }>`
* **Default:** `[{ auto: true }]` (auto-inferred)

Vite Task automatically detects which files are used by a command (see [Automatic File Tracking](https://viteplus.dev/guide/cache.html#automatic-file-tracking)). The `input` option can be used to explicitly include or exclude certain files.
...
### `cwd` [​](https://viteplus.dev/config/run.html#cwd)
